...
def top_up(user_id):
    ...
    try:
        ...
        saldo_baru = get_coin_balance(user_id)
        print(f"[SUCCESS] Top-up {koin} koin berhasil!")
        if saldo_baru is not None:
            print(f"[INFO] Saldo koin Anda sekarang: {saldo_baru} koin.")
    except Exception as e:
...
def sudah_beli(user_id, chapter_id):
    ...
    Kalau cek gagal (misal error koneksi), error dilempar lagi ke pemanggil
    supaya proses beli ikut dibatalkan -- bukan dianggap diam-diam 'belum beli'.
    """
    with db_cursor() as (conn, cursor):
        cursor.execute(
        ...
        return cursor.fetchone() is not None  # True = sudah beli
...
def beli_bab(user_id):
    ...
    try:
        ...
        print("[ERROR] ID Chapter harus berupa angka.")
        return
    ...
    try:
        with db_cursor() as (conn, cursor):
            ...
            saldo_sekarang = user[0] if user else 0

            if not user or saldo_sekarang < coin_cost:
                print(
                    f"[FAILED] Koin tidak cukup, saldo koin Anda sisa {saldo_sekarang} koin. "
                    f"Top up kembali jika ingin membaca chapter ini."
                )
                return
            ...
            sisa_saldo = saldo_sekarang - coin_cost
            print(f"[SUCCESS] Pembelian berhasil! Akses terbuka. -{coin_cost} koin.")
            print(f"[INFO] Sisa saldo koin Anda: {sisa_saldo} koin.")
    except Exception as e:
...
def request_withdrawal(author_id):
    ...
    try:
        ...
        print("[ERROR] Jumlah koin harus berupa angka.")
        return

    if jumlah <= 0:
        print("[ERROR] Jumlah koin harus lebih besar dari 0.")
        return
...
def menu_transaksi(user_id):
    while True:
        ...
        elif pilihan == '5': cek_saldo(user_id)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("[ERROR] User ID harus berupa angka.")
